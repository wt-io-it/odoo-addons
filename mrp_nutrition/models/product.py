...
class ProductTemplate(models.Model):
            ...
            if len(self.bom_ids.ids) > 1:
                ...
                for bom in [self.bom_ids[0]]:
                    ...
                    for bom_line in bom.bom_line_ids:
                        bom_ingridients = bom_line.product_id.product_tmpl_id._recursive_bom_ingredients(qty=bom_line.product_qty, uom=bom_line.product_uom, level=level, ingredients=bom_ingridients)
                self.write_nutrition_facts(bom_ingridients, qty=bom.product_qty, uom=bom.product_uom)
            else:
                bom = self.bom_ids
    ...
    def add_to_ingridients_list(self, ingredients, qty=0, uom=False):
        ...
        if uom and uom != self.uom_id:
            qty = uom._compute_qty(uom.id, qty, self.uom_id.id)
